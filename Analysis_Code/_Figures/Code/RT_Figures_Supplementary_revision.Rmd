...
probval.ann<-
  ...
  group_by(expType,symID.f) %>%
  mutate(expN = n()) %>% 
  group_by(expType,symID.f,resp_valence,expN,val_error) %>% 
  summarise(n=n()) %>% 
  mutate(prop=round(n/expN,2))


f.probval<-

    ggplot(aes(x=resp_valence,y=resp_prob/100, color=as.factor(val_error)),data=probval)+
  see::geom_violinhalf(scale="width",color="transparent", fill="grey80",adjust=1.5) +
  
geom_text(
    data = probval.ann,
    aes(x = resp_valence, y = -0.13, label = prop, color = as.factor(val_error)),
    size.unit = "pt",
    size=18,
    show.legend = FALSE
  ) +

  ggdist::stat_dots(color="gray30",position = position_nudge(x=+0.05),overflow="compress", scale=4/10)+
  geom_boxplot(position=position_nudge(x=-0.20), width=0.3,outlier.shape = NA,lwd=1,color=ss.col.viol,fill="transparent")+
  stat_summary(fun.data = "mean_se", geom = "point",fun.args = list(mult = 1),size=1.5,stroke=1.5)+
  stat_summary(fun.data = "mean_se", geom = "errorbar",fun.args = list(mult = 1),width=0.3,size=1.5)+
  theme_classic(base_family = "CalibriLight",base_size = 25)+
  theme(legend.position="none",
      panel.background = element_rect(fill = "white", colour = "black"),
      strip.background = element_blank())+
  scale_y_continuous(expand = expansion(mult = .2),breaks=c(0,0.25, 0.5, 0.75,1))+
